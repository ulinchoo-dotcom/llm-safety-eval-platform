...
- 50 条人工编写的 v1 种子用例 + 2200 条 v2 样本库（`examples/seed-cases-v2.json`）：22 子类 × 8 手法全覆盖，程序化扩增自 v1（`scripts/expand_seed.py`，确定性可复现）；`minor_sexual_content` 子类按内容政策不生成；标签为模板规则赋予、未经人工逐条复核。
...
**双人独立标注（v2 抽样 200 条）**：作者模板标注 × DeepSeek 独立标注，200/200 一致，**Cohen's Kappa = 1.0（pass）**。保留意见如实标注：第二标注员是 LLM 不是人类（M1 的人类双标未完成），且模板样本边界清晰，κ=1.0 不能外推到模糊用例；下一版应补充灰色地带用例再测。明细见 `reports/dual-label-v2.json`。
...
- M1：完成 23 子类四段式手册的人工审核、500 条种子样本和独立双标。当前有分类配置、手册模板、50 条手写种子 + 2200 条模板扩增样本、以及一次「人 + LLM」双标（κ=1.0）；缺手册人工审核与**两个人类**的独立双标，M1 未验收。
