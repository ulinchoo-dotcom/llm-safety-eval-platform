...
模拟模型名称不是实际模型产品。其输出固定，用量与费用为明确标识的模拟零值。仓库仅附 3 条无害正常对照用例，用于验证系统接线，不代表 500 条种子集、2000 条样本库或任何模型安全结论。
...
- M1：完成 23 子类四段式手册的人工审核、500 条种子样本和独立双标。当前只有分类配置和手册模板，M1 未验收。
